...
public class markerTests extends GeneralTestSuiteFromMarkers {
	...
	public static abstract class markerTestsTestCase extends PhotranWorkspaceTestCase {
		...
		static KeywordCheckAction kwc = new KeywordCheckAction();
		...
		public void testSingleMarker() throws CoreException {
			...
			Token t = tempAST.findFirstTokenOnLine(linenum);

			kwc.generateMarker(t); // User story 4 function
			...
			assertEquals(t.getLine(), problems[0].getAttribute(IMarker.LINE_NUMBER));
			assertTrue(problems[0].getAttribute(IMarker.MESSAGE).toString().contains(t.getText()));
		...
		public void testNullMarker() throws CoreException {
			...
			kwc.generateMarker(null); // User story 4 function
		...
		public void testManyMarker() throws CoreException {
			...
			Token t1 = tempAST.findFirstTokenOnLine(linenum);
			Token t2 = tempAST.findLastTokenOnLine(linenum);

			kwc.generateMarker(t1); // User story 4 function
			kwc.generateMarker(t2); // User story 4 function
			problems = file.findMarkers(IMarker.PROBLEM, true, depth);
			...
			assertTrue((problems[0].getAttribute(IMarker.MESSAGE).toString().contains(t1.getText()) || problems[0]
					.getAttribute(IMarker.MESSAGE).toString().contains(t2.getText())));

			// The marker in problem[1] can be marker for t1 or t2
			assertTrue((problems[1].getAttribute(IMarker.MESSAGE).toString().contains(t1.getText()) || problems[1]
					.getAttribute(IMarker.MESSAGE).toString().contains(t2.getText())));
			assertEquals(IMarker.SEVERITY_WARNING, problems[0].getAttribute(IMarker.SEVERITY));
